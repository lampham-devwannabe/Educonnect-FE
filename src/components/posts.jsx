...
const AllPosts = () => {
  ...
  const [newPost, setNewPost] = useState({ title: "", content: "", image: null, thumbnail: "" });
  const {
    posts,
    setPosts,
    fetchPosts,
  ...
  const handleCreatePost = async (e) => {
    ...
    if (newPost.image) {
      try {
        uploadedImageUrl = await uploadImagePromised(newPost.image, setUploading, setUploadProgress);
      } catch (error) {
